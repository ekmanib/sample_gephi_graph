    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
            ...
            if (attr != image_attribute) {
                d = d.replace(/\(([^)]+)\)/g, (_, content) => { return '<strong>(' + content + ')</strong>' });
                h = '<span><strong>' + 'Atributos' + '</strong></span><br/>' + d.split(',').join('<br/>')
            }
        ...
        $GP.info_data.html(e.join("<br/>")) // This is where we set the content for the node attributes
    }
    $GP.info_data.show();
